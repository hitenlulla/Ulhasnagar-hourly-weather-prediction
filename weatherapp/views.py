...
def predictTemparatures():
    ...
    if int(float(curr_time)) <= 23:
            latestData[curr_time] = {'time': convertTimeFormat(curr_time), 'day': today, 'temperature': int(curr_temp), 'icon':getIconName(curr_time)}
    else:
        latestData[curr_time] = {'time': convertTimeFormat(curr_time), 'day': tomorrow, 'temperature': int(curr_temp), 'icon':getIconName(curr_time)}
    ...
    if int(float(next_time)) <= 23:
        predictedData[next_time] = {'time': convertTimeFormat(next_time),'day': today, 'temperature': int(next_temp), 'icon':getIconName(next_time)}
    else:
        predictedData[next_time] = {'time': convertTimeFormat(next_time), 'day': tomorrow, 'temperature': int(next_temp), 'icon':getIconName(next_time)}
    # predictedData[next_time] = int(next_temp)
# Iterations
    hour_ctr = 1
    output_number_of_hours = 12
    day_change = False
    while hour_ctr != output_number_of_hours:
        if next_time == '23.00':
            day_change = True
        next_time = getTimeNext(next_time)
        prev_temp = prevData[next_time]
        ...
        # predictedData[next_time] = int(next_temp)
        hour_ctr += 1
    return latestData, predictedData

def home(request):
    latest_data, weather_prediction_data = predictTemparatures()
   
    print(weather_prediction_data)

    return render(request, 'home.html', {'latest_data': latest_data,'prediction': weather_prediction_data})